...
erpnext.taxes_and_totals = erpnext.stock.StockController.extend({
	...
	calculate_item_values: function() {
		...
		if (!this.discount_amount_applied) {
			...
			$.each(this.frm.doc["tablamateriales"] || [], function(i, item) {
				frappe.model.round_floats_in(item);
				item.net_rate = item.rate;
				item.amount = flt(item.rate * item.qty, precision("amount", item));
				item.net_amount = item.amount;
			...
			$.each(this.frm.doc["tablaservicio"] || [], function(i, item) {
				frappe.model.round_floats_in(item);
				item.net_rate = item.rate;
				item.amount = flt(item.rate * item.qty, precision("amount", item));
				item.net_amount = item.amount;
			...
			$.each(this.frm.doc["tablarrhh"] || [], function(i, item) {
				frappe.model.round_floats_in(item);
				item.net_rate = item.rate;
				item.amount = flt(item.rate * item.qty, precision("amount", item));
				item.net_amount = item.amount;
			...
			$.each(this.frm.doc["tablaepp"] || [], function(i, item) {
				frappe.model.round_floats_in(item);
				item.net_rate = item.rate;
				item.amount = flt(item.rate * item.qty, precision("amount", item));
				item.net_amount = item.amount;
	...
	calculate_net_total: function() {
		var me = this;
		this.frm.doc.total = this.frm.doc.base_total =this.frm.doc.base_total_materiales=this.frm.doc.base_total_servicios=this.frm.doc.base_total_rrhh=this.frm.doc.base_total_epp= this.frm.doc.net_total = this.frm.doc.base_net_total =this.frm.doc.total_alimentacion=this.frm.doc.total_traslado=this.frm.doc.total1=this.frm.doc.total2=this.frm.doc.total3= 0.0;
		...
		$.each(this.frm.doc["tablamateriales"] || [], function(i, item) {
			me.frm.doc.total += item.amount;
			me.frm.doc.base_total_materiales += item.base_amount;
			
			me.frm.doc.net_total += item.net_amount;
			me.frm.doc.base_net_total += item.base_net_amount;
			me.frm.doc.total3 += item.base_amount;
			me.frm.doc.total2 += item.base_amount;	
			me.frm.doc.total1 += item.base_amount;	
		});
		
		
		$.each(this.frm.doc["tablaservicio"] || [], function(i, item) {
			me.frm.doc.total += item.amount;			
			if(item.item_name == 'Traslado')
			{
				me.frm.doc.total3 += item.base_amount;
			}
			if(item.item_name == 'Alimentación')
			{
				me.frm.doc.total3 += item.base_amount;
				me.frm.doc.total2 += item.base_amount;	
			}
			me.frm.doc.base_total_servicios += item.base_amount;		
			me.frm.doc.net_total += item.net_amount;
			me.frm.doc.base_net_total += item.base_net_amount;
		});
		...
		$.each(this.frm.doc["tablarrhh"] || [], function(i, item) {
			me.frm.doc.total += item.amount;
			me.frm.doc.base_total_rrhh += item.base_amount;
			me.frm.doc.net_total += item.net_amount;
			me.frm.doc.base_net_total += item.base_net_amount;
			me.frm.doc.total3 += item.base_amount;
			me.frm.doc.total2 += item.base_amount;	
		...
		$.each(this.frm.doc["tablaepp"] || [], function(i, item) {
			me.frm.doc.total += item.amount;
			me.frm.doc.base_total_epp += item.base_amount;
			me.frm.doc.net_total += item.net_amount;
			me.frm.doc.base_net_total += item.base_net_amount;
			me.frm.doc.total3 += item.base_amount;
			me.frm.doc.total2 += item.base_amount;	
	...
	calculate_taxes: function() {
		...
		$.each(this.frm.doc["tablamateriales"] || [], function(n, item) {
			var item_tax_map = me._load_item_tax_rate(item.item_tax_rate);
			...
			$.each(me.frm.doc["taxes"] || [], function(i, tax) {
				...
				if (tax.charge_type == "Actual") {
					actual_tax_dict[tax.idx] -= current_tax_amount;
					if (n == me.frm.doc["tablamateriales"].length - 1) {
						current_tax_amount += actual_tax_dict[tax.idx]
					}
				}
